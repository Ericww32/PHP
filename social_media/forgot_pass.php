...
if(isset($_POST["e"])){
	...
	if($numrows > 0){
		...
		$hashTempPass = md5($tempPass); // here is another encryption that we can change
		$sql = "UPDATE useroptions SET temp_pass='$hashTempPass' WHERE username='$u' LIMIT 1";
	    $query = mysqli_query($db_conx, $sql);
		$to = "$e";
		$from = "Do_Not_Respond@PivotTester.x10.bz";
		$headers ="From: $from\n";
		$headers .= "MIME-Version: 1.0\n";
		$headers .= "Content-type: text/html; charset=iso-8859-1 \n";
		$subject ="Intern Persuit Temporary Password";
		$msg = '<h2>Hello '.$u.'</h2>
        <p>This is an automated message from Intern Pursuit. If you did not recently initiate the Forgot Password process, please disregard this email.</p>
        ...
        <p><a href="http://www.PivotTester.x10.bz/forgot_pass.php?u='.$u.'&p='.$hashTempPass.'">Click here now to apply the temporary password shown below to your account</a></p>
